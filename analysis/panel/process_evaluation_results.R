...
process_res <- function(eval_tb_list, return_true = F) {
        if (return_true) {
                eval_tb_list_true = map(eval_tb_list, "true")
                tb_true_all = bind_rows(map(1:length(eval_tb_list_true), function(j) {
                        mutate(eval_tb_list_true[[j]], j = j)
                }))
                ...
                eval_tb_list_recon = map(eval_tb_list, "recon")
                tb_recon_all = bind_rows(map(1:length(eval_tb_list_recon), function(j) {
                        mutate(eval_tb_list_recon[[j]], j = j)
                }))
